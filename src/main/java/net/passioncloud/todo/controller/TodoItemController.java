...
public class TodoItemController {
    ...
    private void addAttributesForIndex(Model model, ListFilter listFilter) {
        ...
        model.addAttribute("timeSpent", todoItemRepository.sumtimeSpent());
    }
    ...
    @PostMapping("/time")
    public String stopTime(@ModelAttribute("startTime") LocalDateTime startTime,Model model) {
    ...
    public record TodoItemDto(long id, String title, boolean isCompleted , Long timeSpent) {
        static TodoItemDto fromTodoItem(TodoItem todoItem) {
            return new TodoItemDto(
                    todoItem.getId(),
                    todoItem.getTitle(),
                    todoItem.isCompleted(),
                    todoItem.gettimeSpent()
            );
        }
    }
...
//    }
    ...
    @PutMapping("/{id}/time")
    public String updateTimeSpent(@PathVariable("id") Long id, @RequestParam("timeSpent") long timeSpent) {
        TodoItem todoItem = todoItemRepository.getById(id);
        todoItem.settimeSpent(timeSpent);
        todoItemRepository.save(todoItem);
        return "redirect:/";
    }

    @PostMapping(headers = "HX-Request", value = "/{id}/time")
    public String htmxUpdateTimeSpent(@PathVariable("id") Long id, @RequestParam("timeSpent") long timeSpent,Model model, HttpServletResponse response) {
        TodoItem todoItem = todoItemRepository.getById(id);
        todoItem.settimeSpent(timeSpent);
        todoItemRepository.save(todoItem);
        model.addAttribute("item", TodoItemDto.fromTodoItem(todoItem));
